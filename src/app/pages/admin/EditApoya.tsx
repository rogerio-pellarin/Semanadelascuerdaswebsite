...
interface ApoyaContent {
  es: ApoyaContentLang;
  en: ApoyaContentLang;
  pt: ApoyaContentLang;
}

export default function EditApoya() {
  const [content, setContent] = useState<ApoyaContent | null>(null);
  ...
  const fetchContent = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-65077a1f/apoya-content`,
        {
          headers: {
  ...
  const updateField = (section: keyof ApoyaContentLang, field: string, value: any) => {
    if (!content) return;
    setContent({
      ...content,
      [currentLang]: {
  ...
  const updateImpactItem = (index: number, field: string, value: string) => {
    if (!content) return;
    const newItems = [...content[currentLang].impact.items];
  ...
  return (
    ...
    >
      ...
      {/* Impact Section */}
      <CMSSection title="El Impacto de tu Donación">
        <CMSField
          label="Título de Sección"
          value={langContent.impact.title}
          onChange={(value) => updateField('impact', 'title', value)}
        />

        <div className="space-y-6">
          {langContent.impact.items.map((item, index) => (
            <div key={item.id} className="rounded-sm border border-white/5 bg-black/50 p-4">
              <h3 className="mb-3 text-sm font-medium text-[#D4AF37]">Nivel {index + 1}</h3>

              <div className="space-y-3">
                <div>
                  <label className="mb-1 block text-xs text-white/40">Monto</label>
                  <input
                    type="text"
                    value={item.amount}
      ...
      {/* Why Section */}
      <CMSSection title="¿Por Qué Donar?">
        <CMSField
          label="Título de Sección"
          value={langContent.why.title}
          onChange={(value) => updateField('why', 'title', value)}
        />
        <CMSField
          label="Contenido"
          value={langContent.why.content}
          onChange={(value) => updateField('why', 'content', value)}
          type="textarea"
        />
      </CMSSection>

      {/* Recognition Section */}
      <CMSSection title="Reconocimiento">
        <CMSField
          label="Título de Sección"
          value={langContent.recognition.title}
          onChange={(value) => updateField('recognition', 'title', value)}
        />
        <CMSField
          label="Contenido"
          value={langContent.recognition.content}
          onChange={(value) => updateField('recognition', 'content', value)}
          type="textarea"
        />
      </CMSSection>
    </CMSEditor>
