...
interface ProgramaContent {
  es: ProgramaContentLang;
  en: ProgramaContentLang;
  pt: ProgramaContentLang;
}

export default function EditPrograma() {
  const [content, setContent] = useState<ProgramaContent | null>(null);
  ...
  const fetchContent = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-65077a1f/programa-content`,
        {
          headers: {
  ...
  const updateField = (section: keyof ProgramaContentLang, field: string, value: any) => {
    if (!content) return;
    setContent({
      ...content,
      [currentLang]: {
  ...
  const updateStructureItem = (index: number, field: string, value: string) => {
    if (!content) return;
    const newItems = [...content[currentLang].structure.items];
  ...
  const updateBenefitItem = (index: number, value: string) => {
    if (!content) return;
    const newItems = [...content[currentLang].benefits.items];
  ...
  return (
    <CMSEditor
      title="Editar Página - El Programa"
    ...
    >
      ...
      {/* Structure Section */}
      <CMSSection title="Estructura del Programa">
        <CMSField
          label="Título de Sección"
          value={langContent.structure.title}
          onChange={(value) => updateField('structure', 'title', value)}
        />

        <div className="space-y-6">
          {langContent.structure.items.map((item, index) => (
            <div key={item.id} className="rounded-sm border border-white/5 bg-black/50 p-4">
              <h3 className="mb-3 text-sm font-medium text-[#D4AF37]">Item {index + 1}</h3>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    type="text"
                    value={item.title}
                    onChange={(e) => updateStructureItem(index, 'title', e.target.value)}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => updateStructureItem(index, 'description', e.target.value)}
                    rows={2}
                    className="w-full rounded-sm border border-white/10 bg-black px-3 py-2 text-sm text-white"
                  />
                </div>
                ...
                <div>
                  <label className="mb-1 block text-xs text-white/40">
                    Icono (user, users, book, music)
                  </label>
                  <input
                    type="text"
                    value={item.icon}
                    onChange={(e) => updateStructureItem(index, 'icon', e.target.value)}
                    className="w-full rounded-sm border border-white/10 bg-black px-3 py-2 text-sm text-white"
                  />
                </div>
              </div>
      ...
      {/* Benefits Section */}
      <CMSSection title="Beneficios">
        <CMSField
          label="Título de Sección"
          value={langContent.benefits.title}
          onChange={(value) => updateField('benefits', 'title', value)}
        />

        <div className="space-y-3">
          {langContent.benefits.items.map((item, index) => (
            <div key={index}>
              <label className="mb-1 block text-xs text-white/40">Beneficio {index + 1}</label>
              <input
                type="text"
                value={item}
      ...
      {/* Schedule Section */}
      <CMSSection title="Horario">
        <CMSField
          label="Título"
          value={langContent.schedule.title}
          onChange={(value) => updateField('schedule', 'title', value)}
        />
        <CMSField
          label="Descripción"
          value={langContent.schedule.description}
          onChange={(value) => updateField('schedule', 'description', value)}
          type="textarea"
        />
        <CMSField
          label="Horario Diario"
          value={langContent.schedule.daily}
          onChange={(value) => updateField('schedule', 'daily', value)}
        />
      </CMSSection>
    </CMSEditor>
  );
